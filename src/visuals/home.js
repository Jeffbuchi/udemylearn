...
function HomePage () {
    ...
    for (let i = 0; i < popularCourse.length; i++) {
        courseList.push(
            <NavLink to ={"/content/" + popularCourse[i].ID} className = "course rel" key = {"popular-course-" + i}>
                <div className = "blk rel" style = {{
                    background : "lightgrey url(" + popularCourse[i].poster + ")"
                }}>

<div className = "user abs flex">
                        <div className = "inset">
                            <img src = {popularCourse[i].tutor.dp} className = "block" />
                        </div>
                        <div className = "meta">
                            <h2 className = "name s13 fontz">{popularCourse[i].tutor.name}</h2>
                            <h2 className = "username s13 fontz">{popularCourse[i].tutor.userName}</h2>
                        </div>
                    </div>
                    <div className = "dura abs">
                    <h2 className = "username s13 fontz">{popularCourse[i].duration}</h2>
                    </div>
                    <div className = "course-title abs">
                    <h2 className = "username s15 fontb">{popularCourse[i].title}</h2>
                    </div>
